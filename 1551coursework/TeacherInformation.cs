...
namespace _1551coursework
{
    public partial class TeacherInformation : Form
    {
        ...
        public TeacherInformation(string username)
        {
            InitializeComponent();
        ...
        private void TeacherInformation_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    // SQL query to fetch teacher information based on username
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            // Displaying teacher information in the form's text boxes
                            TeacherID.Text = reader.GetValue(0).ToString();
                            TeacherFN.Text = reader.GetValue(1).ToString();
                            TeacherLN.Text = reader.GetValue(2).ToString();
                            TeacherEmail.Text = reader.GetValue(3).ToString();
                            TeacherTelephone.Text = reader.GetValue(4).ToString();
                            TeacherSalary.Text = reader.GetValue(8).ToString();
                            TeacherSubject1.Text = reader.GetValue(9).ToString();
                            TeacherSubject2.Text = reader.GetValue(10).ToString();
                        }
                    }
                }
            }
